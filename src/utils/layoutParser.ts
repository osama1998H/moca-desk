...
function makeColumn(): ParsedColumn {
  return { fields: [] };
}
 ...
 */
export function parseLayout(fields: FieldDef[]): ParsedLayout {
  if (fields.length === 0) return { tabs: [] };

  const tabs: ParsedTab[] = [];
  let currentTab = makeTab();
  // makeTab/makeSection always initialize with one section/column
  let currentSection: ParsedSection = currentTab.sections[0]!;
  let currentColumn: ParsedColumn = currentSection.columns[0]!;
    ...
    switch (field.field_type) {
      ...
      case "SectionBreak": {
        ...
        const newSection = makeSection(
          field.layout_label || field.label || "",
          field.collapsible ?? false,
          field.collapsed_by_default ?? false,
        );
  ...
  for (const tab of tabs) {
    ...
    const meaningful = tab.sections.filter(
      (s) => sectionHasFields(s) || s.label,
    );
  ...
  const nonEmptyTabs = tabs.filter((t) =>
    t.sections.some(sectionHasFields),
  );
