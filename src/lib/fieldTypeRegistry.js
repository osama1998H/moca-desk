 ...
 */
export function registerFieldType(name, component) {
    if (import.meta.env.DEV && name in FIELD_TYPE_MAP) {
        console.warn(`[moca] registerFieldType: "${name}" collides with a built-in field type. ` +
 ...
 */
export function getCustomFieldType(name) {
